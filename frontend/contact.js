...
function applyBreadcrumb() {
    ...
    if (el) {
        const img = SITE_CONFIG.breadcrumbs.contact;
        if (img) {
...
function renderFooterContact(company) {
    ...
    // ── Contact section: primary phone
    const contPhonesEl = document.getElementById("contact-phones");
    ...
    // ── Contact section: primary address
    const contAddressEl = document.getElementById("contact-address");
    if (contAddressEl && company.addresses?.length) {
        const primary = company.addresses.find(a => a.is_primary) || company.addresses[0];
        const parts = [
            field(primary, "address_line1"),
            field(primary, "city"),
